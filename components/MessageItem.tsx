...
const MessageItemComponent: React.FC<MessageItemProps> = ({ message, isNew }) => {
  ...
  return (
    <div className={`relative mb-5 sm:mb-6 md:mb-8 transition-all duration-500 ${isNew ? 'animate-slide-up' : ''} group`}>
      ...
      <div 
        className="absolute -inset-1 opacity-5 blur-md transition-all group-hover:opacity-10 pointer-events-none"
        style={{ backgroundColor: config.color }}
      ></div>

      <div className={`glass-panel p-4 sm:p-5 md:p-6 lg:p-7 rounded-xl sm:rounded-2xl border-l-[4px] sm:border-l-[5px] md:border-l-[6px] shadow-xl relative overflow-hidden ${isSOS ? 'animate-pulse-sos border-[#ff2e2e]' : 'border-white/10'}`}
...
export const MessageItem = memo(MessageItemComponent, (prev, next) => {
  return prev.isNew === next.isNew && prev.message === next.message;
});
